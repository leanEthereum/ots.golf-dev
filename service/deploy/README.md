...
3. Job storage at `/srv/ots-work`, at most 64 GiB. The installer creates it: a fully allocated
   ...
   `2770`. Its fstab options include `nodiscard,X-fstrim.notrim`: scheduled `fstrim` otherwise
   punches holes in the backing image and makes subsequent jobs fail storage preflight.
   The installer also restores the reservation with `fallocate` when updating older hosts.
   The verifier accepts a loop-backed volume only when its image is fully allocated, so the
...
1. As the verifier user, run the isolation probe, build the internal lower-bound witnesses of the
   ...
   sudo -u ots -H bash -c 'set -a; . /etc/ots/public.env; set +a
     ...
     for t in lower-generality-1 upper-compressions upper-riscv upper-leanisa upper-riscv-hint; do
...
3. Confirm `OTS_PHONY=0` for both services. Every board without a real verified record shows
   ...
   whole-word lower board, Upper bound, RISC-V upper bound, leanISA upper bound and Hinted RISC-V
   upper bound. Existing demo rows remain stored but hidden and cannot participate in record
   decisions.
...
The active contract contains whole-word lower bounds and the four upper-bound tracks. Removed
